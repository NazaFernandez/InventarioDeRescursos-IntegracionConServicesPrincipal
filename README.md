...
# Azure Resource Inventory - Custom (Service Principal Auth)

## 📊 Diagrama de Arquitectura
El siguiente diagrama explica el proceso lógico tanto de la instalación como de la ejecución modificada:
...
graph TD
    ...
    subgraph Fase1 ["Fase 1: Instalación (Install-ARI-Custom.ps1)"]
        A1("Inicio: Ejecutar Install-ARI-Custom.ps1"):::startEnd --> A2["Detectar versión de PS y obtener ruta"]:::process
    ...
    subgraph Fase2 ["Fase 2: Ejecución de Inventario (Invoke-ARI)"]
        B1("El usuario abre PowerShell y escribe 'Invoke-ARI'"):::userAction --> B2{"¿Se pasaron parámetros\nde autenticación?"}:::decision
        B2 -- Sí --> B3["Ejecutar flujo original interactivo"]:::process
        B2 -- No --> B4["Desplegar Prompt Personalizado\n(Modificación en Connect-ARILoginSession)"]:::process
        B4 --> B5("El usuario ingresa:\nApp ID, Secret, Tenant ID, Subs ID"):::userAction
        B5 --> B6["Ejecutar: Connect-AzAccount -ServicePrincipal"]:::process
        B6 --> B7{"¿Autenticación\nExitosa?"}:::decision
        B7 -- No --> B8("Error: Credenciales Inválidas"):::error
        B7 -- Sí --> B9{"¿Permisos de lectura\n(Reader) válidos?"}:::decision
        B9 -- No --> B10("Error: ARI falla al extraer recursos"):::error
        B9 -- Sí --> B11["Orquestación normal de ARI"]:::process
        B3 --> B11
        B11 --> B12("Generar Tablero Excel (.xlsx)"):::success
    end
