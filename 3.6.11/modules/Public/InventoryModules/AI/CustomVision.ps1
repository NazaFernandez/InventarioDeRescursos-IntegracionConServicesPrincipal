<#
.Synopsis
Inventory for Azure Custom Vision
...
Excel Sheet Name: Custom Vision

.Link
https://github.com/microsoft/ARI/Modules/Public/InventoryModules/AI/CustomVision.ps1
...
If ($Task -eq 'Processing')
{
    ...
    $CustomVision = $Resources | Where-Object {$_.TYPE -eq 'microsoft.cognitiveservices/accounts' -and $_.Kind -like 'CustomVision.*'}

    <######### Insert the resource Process here ########>

    if($CustomVision)
        {
            $tmp = foreach ($1 in $CustomVision) {
                $ResUCount = 1
...
Else
{
    ...
    if($SmaResources)
    {
        ...
        Export-Excel -Path $File -WorksheetName 'Custom Vision' -AutoSize -MaxAutoSizeRows 100 -ConditionalText $condtxt -TableName $TableName -TableStyle $tableStyle -Style $Style
