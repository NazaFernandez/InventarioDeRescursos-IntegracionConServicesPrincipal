<#
.Synopsis
Inventory for Azure Content Safety
...
Excel Sheet Name: Content Safety

.Link
https://github.com/microsoft/ARI/Modules/Public/InventoryModules/AI/ContentSafety.ps1
...
If ($Task -eq 'Processing')
{
    ...
    $ContentSafety = $Resources | Where-Object {$_.TYPE -eq 'microsoft.cognitiveservices/accounts' -and $_.Kind -eq 'ContentSafety'}

    <######### Insert the resource Process here ########>

    if($ContentSafety)
        {
            $tmp = foreach ($1 in $ContentSafety) {
                $ResUCount = 1
...
Else
{
    ...
    if($SmaResources)
    {
        ...
        Export-Excel -Path $File -WorksheetName 'Content Safety' -AutoSize -MaxAutoSizeRows 100 -ConditionalText $condtxt -TableName $TableName -TableStyle $tableStyle -Style $Style
