...
Excel Sheet Name: Language

.Link
https://github.com/microsoft/ARI/Modules/Public/InventoryModules/AI/TextAnalytics.ps1
...
If ($Task -eq 'Processing')
{
    ...
    $Language = $Resources | Where-Object {$_.TYPE -eq 'microsoft.cognitiveservices/accounts' -and $_.Kind -eq 'TextAnalytics'}

    <######### Insert the resource Process here ########>

    if($Language)
        {
            $tmp = foreach ($1 in $Language) {
                $ResUCount = 1
                    ...
                    foreach ($pv in $pvt)
                        {
                            ...
                            foreach ($Tag in $Tags) {
                                $obj = @{
                                    ...
                                    'Quota Limit'                               = $data.quotaLimit.rules.'count';
