<#
.Synopsis
Inventory for Azure Speech Service
...
Excel Sheet Name: Speech Service

.Link
https://github.com/microsoft/ARI/Modules/Public/InventoryModules/AI/SpeechService.ps1
...
If ($Task -eq 'Processing')
{
    ...
    $SpeechService = $Resources | Where-Object {$_.TYPE -eq 'microsoft.cognitiveservices/accounts' -and $_.Kind -eq 'SpeechServices'}

    <######### Insert the resource Process here ########>

    if($SpeechService)
        {
            $tmp = foreach ($1 in $SpeechService) {
                $ResUCount = 1
